...
func start_training(charТame, charСlass string) string {
	...
	for cmd != "skip" {
		...
		if cmd == "attack" {
			fmt.Println(attack(charТame, charСlass))
		}

		if cmd == "defence" {
			fmt.Println(defence(charТame, charСlass))
		}

		if cmd == "special" {
			fmt.Println(special(charТame, charСlass))
		}
	}
